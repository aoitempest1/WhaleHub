...
local Version = "COUNTER BLOX VIP MADE BY AoiTempest"
...
--// ESP
Visuals:CreateSection(" Highlight ESP")
Visuals:CreateToggle({
...
Visuals:CreateColorPicker({
    ...
    Flag = "HighlightColor",
    Callback = function(c) ESPConfig.Color = c end,
})
Visuals:CreateDropdown({
    Name = "Team Filter",
    Options = {"Enemies Only", "Teammates Only", "Both Teams", "Everyone (ignore team)"},
    CurrentOption = "Enemies Only",
    Flag = "HighlightTeamFilter",
    Callback = function(opt) ESPConfig.TeamFilter = opt end,
})
...
local function shouldShow(player)
    ...
    if ESPConfig.TeamFilter == "Enemies Only" then
        return myTeam and targetTeam and myTeam ~= targetTeam
    elseif ESPConfig.TeamFilter == "Teammates Only" then
        return myTeam and targetTeam and myTeam == targetTeam
    elseif ESPConfig.TeamFilter == "Both Teams" then
        return myTeam and targetTeam
    elseif ESPConfig.TeamFilter == "Everyone (ignore team)" then
        return true
    end
    return false
end
...
for _, player in pairs(Players:GetPlayers()) do
    if player ~= LocalPlayer then
        if player.Character then updateHighlight(player) end
        player.CharacterAdded:Connect(function()
            task.wait(1)
            updateHighlight(player)
        end)
